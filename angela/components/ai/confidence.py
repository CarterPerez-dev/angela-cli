# Reduce score for malformed flags
        if malformed_flags > 0:
            validation_score = max(0.3, validation_score - (malformed_flags * 0.15))
        
        # Check for redundant flags (same flag repeated)
        unique_flags = set(flags)
        if len(flags) > len(unique_flags):
            # There are duplicate flags
            validation_score = max(0.3, validation_score - 0.1)
        
        # Command-specific flag validations
        if base_command == 'find' and '-name' in flags:
            # Check if there's a pattern following -name
            find_args = command_analysis.args
            name_flag_index = flags.index('-name')
            # Find position of -name in the original command tokens
            command_tokens = shlex.split(command)
            try:
                name_token_index = command_tokens.index('-name')
                if name_token_index + 1 >= len(command_tokens) or command_tokens[name_token_index + 1].startswith('-'):
                    # No pattern after -name
                    validation_score = max(0.3, validation_score - 0.2)
            except (ValueError, IndexError):
                pass
        
        # Check if flags are placed correctly (not after arguments for many commands)
        if base_command not in ['tar', 'ps', 'netstat', 'openssl']:
            args_before_flags = False
            flag_found = False
            
            for token in shlex.split(command)[1:]:
                if token.startswith('-'):
                    flag_found = True
                elif flag_found and not token.startswith('-'):
                    args_before_flags = False
                elif not flag_found and not token.startswith('-'):
                    args_before_flags = True
            
            if args_before_flags:
                # Flags after arguments for commands that typically don't allow it
                validation_score = max(0.3, validation_score - 0.1)
    ...
    def _check_contextual_relevance(
        ...
        """
        # Get context manager and session manager
        ...
        # Check for operation relevance based on recent activity
        ...
        # Check for environment variable relevance
    ...
    def _check_path_relevance(
        ...
        for entity in path_entities:
            ...
            if path.exists():
                ...
                if project_root and str(project_root) in str(path):
        ...
        if path_entities:
            ...
            path_score = 0.5 + (path_validity_ratio * 0.5)
        
        # Check for recent file/directory usage overlap
        recent_match_score = 0.0
        recent_files = [path for path, _ in current_state.recent_files]
        recent_dirs = [path for path, _ in current_state.recent_dirs]
        ...
        for entity in path_entities:
            ...
            for recent in recent_files:
                if path == recent or str(path).startswith(str(recent)):
    ...
    def _check_operation_relevance(
        ...
        """Check if operation type is relevant to recent activities."""
        # Start with a moderate score
        operation_score = 0.7
        ...
        subcategory = command_analysis.subcategory
        
        # Check recent commands for continuity
        recent_commands = current_state.recent_commands
        
        if recent_commands:
            # Get most recent command
            last_cmd, _ = recent_commands[0]
            last_base = last_cmd.split()[0] if last_cmd.split() else ""
            
            # Check for sensible command sequences
            sequence_score = 0.0
            ...
            elif (last_base in ['touch', 'mkdir', 'vi', 'vim', 'nano', 'emacs'] and 
                  base_command in ['cat', 'ls', 'echo', 'cd', 'chmod']):
                sequence_score += 0.2
                
            # Deletion after view pattern
            elif (last_base in ['cat', 'ls', 'find'] and 
                  base_command in ['rm', 'mv']):
                sequence_score += 0.1
                
            # Git workflow patterns
            elif (last_base == 'git' and base_command == 'git' and
                  ('status' in last_cmd and ('add' in command_analysis.args or 'commit' in command_analysis.args)) or
                  ('add' in last_cmd and 'commit' in command_analysis.args) or
                  ('commit' in last_cmd and 'push' in command_analysis.args)):
                sequence_score += 0.3
                
            # Package management patterns
            elif (last_base in ['npm', 'pip', 'apt', 'brew'] and 
                  (base_command == last_base or base_command in ['cd', 'ls', 'python', 'node'])):
                sequence_score += 0.2
            
            # Combine with operation score
        ...
        if current_state.active_processes:
            ...
            # Check for commands that are relevant to active processes
            # For example, git commands when git is active, docker commands when docker is active
            for process in current_state.active_processes:
                if process.lower() in base_command.lower() or base_command.lower() in process.lower():
                    process_match_score += 0.1
                    break
            
            # Commands related to process management
            if category == CommandCategory.PROCESS:
                process_match_score += 0.1
    ...
    def _check_project_relevance(
        ...
        if project_type == "python":
            if command_analysis.base_command in ['python', 'pip', 'pytest', 'virtualenv', 'venv']:
                project_score += 0.3
            elif any(ext in command_analysis.args for ext in ['.py', '.pyc', 'requirements.txt']):
                project_score += 0.2
                
        elif project_type == "node":
            if command_analysis.base_command in ['node', 'npm', 'yarn', 'npx']:
                project_score += 0.3
            elif any(file in command_analysis.args for file in ['package.json', 'node_modules', '.js', '.ts']):
                project_score += 0.2
                
        elif project_type == "go":
            if command_analysis.base_command in ['go', 'gofmt', 'godoc']:
                project_score += 0.3
            elif any(ext in command_analysis.args for ext in ['.go', 'go.mod', 'go.sum']):
                project_score += 0.2
                
        elif project_type == "ruby":
            if command_analysis.base_command in ['ruby', 'rails', 'bundle', 'gem']:
                project_score += 0.3
            elif any(ext in command_analysis.args for ext in ['.rb', 'Gemfile']):
                project_score += 0.2
                
        elif project_type == "java":
            if command_analysis.base_command in ['java', 'javac', 'mvn', 'gradle']:
                project_score += 0.3
            elif any(ext in command_analysis.args for ext in ['.java', '.jar', 'pom.xml', 'build.gradle']):
                project_score += 0.2
        
        # Git is relevant to all project types
        if command_analysis.base_command == 'git':
            project_score += 0.1
    ...
    def _check_environment_relevance(
        ...
        """Check if command is relevant to current environment variables."""
        # Start with an average score
        env_score = 0.7
        ...
        for arg in command_analysis.args:
            if arg.startswith('# angela/components/ai/confidence.py
"""
...
class ConfidenceScorer:
    ...
    def _check_command_flags(
        ...
        for flag in flags:
            ...
            # Check for short flag format with combined options
                ...
                if base_command in ['find', 'git', 'docker'] and not re.match(r'-[a-zA-Z0-9]+, flag):
                    malformed_flags += 1
                # Check for digit in flags for commands that don't use them
                elif base_command not in ['head', 'tail', 'chmod', 'dd'] and re.search(r'-\D*\d+\D*, flag):
                    malformed_flags += 1):
                var_name = arg[1:]
        ...
        # If command depends heavily on environment variables that
        # are not defined, reduce the score
        for arg in command_analysis.args:
            if arg.startswith('# angela/components/ai/confidence.py
"""
Advanced confidence scoring system for Angela CLI.
